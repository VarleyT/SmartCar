    ...
    while (true) {
        if (USART1_RX_STA & (1 << 15)) {
            if (strstr((const char *) USART1_RX_BUF, "direction")) {
                const char *p = strchr((const char *) USART1_RX_BUF, '=');
                p++;
                if (strstr(p, "stop")) {
            ...
            if (strstr((const char *) USART1_RX_BUF, "pwm")) {
                const char *num1 = strchr((const char *) USART1_RX_BUF, '=');
                const char *num2 = strchr((const char *) USART1_RX_BUF, ',');
                motor.setSpeed(atoi(num1 + 1), atoi(num2 + 1));
            }
            if (strstr((const char *) USART1_RX_BUF, "light")) {
                const char *ch = strchr((const char *) USART1_RX_BUF, '=');
                if (atoi(ch + 1)) {
                    led_r.on();
                    led_g.on();
                    led_b.on();
        ...
        if (count == 3000) {
            ...
            if (data.temperature != 0.0) {
                char buf[6];
                sprintf(buf, "%.2f", data.temperature);
                oled.showString(4 + 16 * 3, 2, buf);
                sprintf(buf, "%.2f", data.humidity);
